  ...
  public static void main(String[] args)
  {
    ...
    while (scanner.hasNextLine())
    {
      ...
      else if (action.equalsIgnoreCase("REQRIDE")) 
      {
        ...
        if (!tmuber.requestRide(id, from, to)) 
          System.out.println(tmuber.getErrorMessage());  
        else  
          System.out.printf("\nRIDE for: %-15s From: %-15s To: %-15s", tmuber.getUser(id).getName(), from, to);
      }
      ...
      else if (action.equalsIgnoreCase("REQDLVY")) 
      {
        ...
        if (!tmuber.requestDelivery(id, from, to, restaurant, foodOrder))
          System.out.println(tmuber.getErrorMessage());  
        else 
          System.out.printf("\nDELIVERY for: %-15s From: %-15s To: %-15s", tmuber.getUser(id).getName(), from, to);
      }
      // Sort users by name
