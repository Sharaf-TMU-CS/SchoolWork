...
public class RemoveSmallLists
{
    ...
    public static void removeSmallLists(ArrayList<ArrayList <Integer>> lists, int minValue) {
        ...
        ArrayList<ArrayList<Integer> > copyList = new ArrayList<ArrayList<Integer>>(lists);
        int removed = 0;
        for (int i=0; i<copyList.size(); i++) {
            int total = 0;
            for (int j=0; j<(copyList.get(i)).size(); j++) {
                total += (copyList.get(i)).get(j);
            }
            if (total < minValue) {
                lists.remove(i-removed);
                removed++;
            }
        }
